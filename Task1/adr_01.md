...
### **Название задачи:** Разработка архитектуры на C1-уровне (контекст системы) для платформы мониторинга поголовья скота.
...
Поэтому решено сделать MVP архитектуры c нуля и запустить как отдельную платформу для мониторинга скота. Сначала планируется реализовать это на ограниченном количестве ферм, а затем потенциально предлагать как SaaS-решение для других компаний (это уже выходит за рамки данного документа).

Требуется разработать C1-уровень (контекст системы) архитектуру для решения данной задачи.
...
- Система разделена на две основные подсистемы:
  - **Центральная система мониторинга ферм** (_Central System_) — отвечает за централизованный сбор и хранение данных с различных ферм, а также за предоставление интерфейсов для пользователей.
  - **Система агента** (_Agent System_) — развёртывается на каждом агенте (ферме) и отвечает за сбор данных с локальных датчиков, камер и систем, а также за обработку данных и взаимодействие с центральной системой. Агентская система обеспечивает автономную работу фермы в случае отсутствия интернет-соединения.
- Центральная система взимодействует с несколькими агентами посредством брокера сообщений (_Message Broker_), что позволяет обеспечить надёжную и масштабируемую коммуникацию между центральной системой и агентами.
- С системой взаимодействуют 3 основных актора:
...
- Система оперативного оповещения дежурного персонала (_On-Site Notification Service_) на ферме вынесена в отдельный "внешний" компонент, так как её реализация может сильно варьироваться в зависимости от конкретной фермы и её инфраструктуры. Разрабатываемая система должна иметь возможность интеграции с разными системами оповещения, будь то SMS, email, мессенджеры или специализированные приложения.


### **Альтернативы**

- Отсутствие центральной системы мониторинга и полная автономность каждой фермы. Однако это усложнит масштабирование системы, перенеся логику агрегации данных и управления множеством агентов на сторону пользовательского приложения.
- Прямое взаимодействие между центральной системой и устройствами на ферме без использования брокера сообщений. Однако это может привести к проблемам с масштабируемостью и надёжностью системы, особенно при увеличении количества ферм.
- Отсутствие агентской системы на фермах - все данные собираются и обрабатываются централизованно. Однако это решение не учитывает требования к автономной работе фермы в случае отсутствия интернет-соединения.
...
- Центральная система становится потенциальной точкой отказа, что требует внедрения механизмов резервирования и отказоустойчивости.
- Данные от агентов к центральной системе могут передаваться с задержками, что может привести к невалидность команд, переданных от центральной системы к агентам. Агенты должны быть способны правильно обрабатывать такие ситуации.
- На ферме нужно будет подготовить инфраструктуру для оповещений, даже в случае отсутствия интернета.
